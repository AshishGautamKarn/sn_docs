...
get_postgresql_connection() {
    ...
    read -p "Database Name [sn_docs]: " DB_NAME
    ...
    if PGPASSWORD="your_password" "$PSQL_CMD" -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "SELECT 1;" >/dev/null 2>&1; then
        ...
        if [[ "$CREATE_DB_USER" =~ ^[Yy]$ ]]; then
            ...
            if PGPASSWORD="your_password" "$PSQL_CMD" -h "$DB_HOST" -p "$DB_PORT" -U "$CURRENT_USER" -c "CREATE DATABASE $DB_NAME;" >/dev/null 2>&1; then
            ...
            if PGPASSWORD="your_password" "$PSQL_CMD" -h "$DB_HOST" -p "$DB_PORT" -U "$CURRENT_USER" -c "CREATE USER $DB_USER WITH PASSWORD '$NEW_USER_PASSWORD';" >/dev/null 2>&1; then
                print_success "User '$DB_USER' created successfully!"
            else
                print_warning "User '$DB_USER' may already exist or creation failed"
            fi
            
            if PGPASSWORD="your_password" "$PSQL_CMD" -h "$DB_HOST" -p "$DB_PORT" -U "$CURRENT_USER" -c "GRANT ALL PRIVILEGES ON DATABASE $DB_NAME TO $DB_USER;" >/dev/null 2>&1; then
            ...
            print_status "Setting up additional permissions..."
            if PGPASSWORD="your_password" "$PSQL_CMD" -h "$DB_HOST" -p "$DB_PORT" -U "$CURRENT_USER" -d "$DB_NAME" -c "GRANT ALL PRIVILEGES ON SCHEMA public TO $DB_USER;" >/dev/null 2>&1; then
                print_success "Schema privileges granted!"
            fi
            
            if PGPASSWORD="your_password" "$PSQL_CMD" -h "$DB_HOST" -p "$DB_PORT" -U "$CURRENT_USER" -d "$DB_NAME" -c "GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO $DB_USER;" >/dev/null 2>&1; then
                print_success "Table privileges granted!"
            fi
            
            if PGPASSWORD="your_password" "$PSQL_CMD" -h "$DB_HOST" -p "$DB_PORT" -U "$CURRENT_USER" -d "$DB_NAME" -c "GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO $DB_USER;" >/dev/null 2>&1; then
                print_success "Sequence privileges granted!"
            fi
            
            if PGPASSWORD="your_password" "$PSQL_CMD" -h "$DB_HOST" -p "$DB_PORT" -U "$CURRENT_USER" -d "$DB_NAME" -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL PRIVILEGES ON TABLES TO $DB_USER;" >/dev/null 2>&1; then
...
check_database_permissions() {
    ...
    local db_name="$3"
    local db_user="$4"
    local db_password="your_password"
...
EOF
    ...
    if [ -n "$db_password" ]; then
        PGPASSWORD="your_password" "$PSQL_CMD" -h "$db_host" -p "$db_port" -U "$db_user" -d "$db_name" -f "$TEMP_SQL" 2>/dev/null
    else
...
EOF
    ...
    if [ -n "$db_password" ]; then
        permission_result=$(PGPASSWORD="your_password" "$PSQL_CMD" -h "$db_host" -p "$db_port" -U "$db_user" -d "$db_name" -f "$TEMP_SQL" 2>/dev/null)
    else
    ...
    if [ -n "$db_password" ]; then
        can_create_check=$(PGPASSWORD="your_password" "$PSQL_CMD" -h "$db_host" -p "$db_port" -U "$db_user" -d "$db_name" -t -c "
            SELECT 
                CASE 
                    WHEN has_database_privilege(current_user, current_database(), 'CREATE') 
                     AND has_schema_privilege(current_user, 'public', 'CREATE') 
...
EOF
    ...
    if [ -n "$db_password" ]; then
        if PGPASSWORD="your_password" "$PSQL_CMD" -h "$db_host" -p "$db_port" -U "$db_user" -d "$db_name" -f "$TEMP_SQL" >/dev/null 2>&1; then
    ...
    local table_count
    if [ -n "$db_password" ]; then
        table_count=$(PGPASSWORD="your_password" "$PSQL_CMD" -h "$db_host" -p "$db_port" -U "$db_user" -d "$db_name" -t -c "
...
setup_environment() {
    ...
    if [ ! -f ".env" ]; then
        ...
        if [ "$USE_POSTGRESQL" = true ]; then
            ...
            sed -i.bak "s/DB_NAME=sn_docs/DB_NAME=$DB_NAME/" .env
            ...
            sed -i.bak "s/DB_NAME=sn_docs/DB_NAME=sn_docs/" .env
